...
public class SplayTree implements Dictionary {
  ...
  private void rotateRight(BinaryTreeNode node) {
    if (node == null ) {
      ...
      return;
        ...
        System.out.println("Illegal call to rotateRight().  You have bug #2.");
        return;
    }
    else if(node.parent.leftChild != node){
        System.out.println("Illegal call to rotateRight().  You have bug #3.");
        return;
    }
  ...
  private void zigZig(BinaryTreeNode node) {
    ...
    if (node == null || node.parent == null) {
        System.out.println("Illegal call to zigZig().  You have bug #3.");
        return;  // Cannot rotate.
      }
    if(node == node.parent.leftChild){
  ...
  private void splayNode(BinaryTreeNode node) {
    ...
    while (node.parent != null) {
    	...
    	else if(node == par.leftChild && par == par.parent.leftChild || node == par.rightChild && par == par.parent.rightChild){
    		zigZig(node);
    	}
    	else{
    		zigZag(node);	
