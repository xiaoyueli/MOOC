...
class SibTreeNode extends TreeNode {
  ...
  public void insertChild(Object item, int c) throws InvalidNodeException {
    // FILL IN YOUR SOLUTION TO PART II HERE.
	  ...
	  myTree.size ++;
	  SibTreeNode nextChild = firstChild;
	  SibTreeNode preSibling = null;
	  int cntChild = 0;
	  while(nextChild != null && cntChild < c - 1 ){
		  preSibling = nextChild;
		  cntChild ++;
		  nextChild = nextChild.nextSibling;
	  }
	  if(c == 1)
	  {
		  preSibling = firstChild;
		  firstChild = newChild;
		  newChild.nextSibling = preSibling;
	  }
	  else if(nextChild != null){
		  preSibling.nextSibling = newChild;
  ...
  public void removeLeaf() throws InvalidNodeException {
    // FILL IN YOUR SOLUTION TO PART III HERE.
	  ...
	  if(firstChild == null){
		  ...
		  valid = false;
		  if(parent != null){
			  SibTreeNode tempSibling = parent.firstChild;
			  SibTreeNode preSibling;  
			  if(tempSibling == this){
				  parent.firstChild = nextSibling;
			  }
			  else{
				  do{
					  preSibling = tempSibling;
					  tempSibling = tempSibling.nextSibling;
				  }while(tempSibling != this);
