...
public class Maze {
  ...
  public Maze(int horizontalSize, int verticalSize) {
    ...
    // Create an array to store all the walls which represent as Items.
    // Mark the hWall with flag 0 and vWall with flag 1.
    int lenWall = horiz * (vert - 1) + (horiz - 1) * vert;
    Item[] walls = new Item[lenWall];
    int cntWall = 0;
    ...
    // Create an two dimension array to store cells.
    // Allocate each cell an index from 0 to vert * horiz - 1 
    int index = 0;
    Item[][] cells = new Item[vert][horiz];
    for(int row = 0; row < vert; row ++){
    	for(int col = 0; col < horiz; col ++){
    		cells[row][col] = new Item(row, col, index);
    		index ++;
    	}
    }
    
    // Create a set to track the cell groups;
    DisjointSets cellSets = new DisjointSets(horiz * vert);
    
    // Shuffle the walls randomly
    while(cntWall > 1){
    	int idx = randInt(cntWall);
    	Item temp = walls[idx];
    	walls[idx] = walls[cntWall - 1];
    	walls[cntWall - 1] = temp;
    	cntWall --;
    }
    
    // deal with the wall by order, get the cells beside the wall.
    // If the two cells have different root, union them in cellSets.
    for(int idx = 0; idx < lenWall; idx ++){
    	Item wall = walls[idx];
    	int wRow = wall.getRow();
    	int wCol = wall.getCol();
    	int c1Row, c1Col, c2Row, c2Col;
    	int whichWall = wall.getIdx();
		c1Row = wRow;
		c1Col = wCol;
		int flag;
    	if(whichWall == 0){
    	...
    	Item cell1 = cells[c1Row][c1Col];
    	Item cell2 = cells[c2Row][c2Col];
		int rcell1 = cellSets.find(cell1.getIdx());
		int rcell2 = cellSets.find(cell2.getIdx());
		if(rcell1 != rcell2){
			cellSets.union(rcell1, rcell2);
			if(flag == 0){
				hWalls[wCol][wRow] = false;
			}
			else{
				vWalls[wCol][wRow] = false;
			}
		}
    ...
    /**
     *  The implementation of DFS method.
     *  Mark cell with 0 to identify this cell has not been visited yet, otherwise mark it with 1.
     */
//	  int index = 0;
//	  Item[][] cells = new Item[vert][horiz];
//	  for(int row = 0; row < vert; row ++){
...
//	  
//	  // Get a root randomly.
//	  Item root = cells[randInt(vert)][randInt(horiz)];
//	  DFS(cells, root);
    
    	
  }
  
  public void DFS(Item[][] map, Item root){
	  if(root.getIdx() == 0){
		  root.setIdx(1);
		  int row = root.getRow();
		  int col = root.getCol();
		  // DFS each neighbor randomly
		  int[] cases = {0, 1, 2 , 3};
		  int len = 4;
		  do{
			  int idx = randInt(len);
			  int num = cases[idx];
			  cases[idx] = cases[len - 1];
			  len --;
			  switch(num){
			  case 0:
				  if(row - 1 >=0 && map[row - 1][col].getIdx() == 0){
					  hWalls[col][row - 1] = false;
					  DFS(map, map[row - 1][col]);
				  }
				  break;
			  case 1:
				  if(row + 1 < vert && map[row + 1][col].getIdx() == 0){
					  hWalls[col][row] = false;
					  DFS(map, map[row + 1][col]);
				  }
				  break;
			  case 2:
				  if(col - 1 >= 0 && map[row][col - 1].getIdx() == 0){
					  vWalls[col - 1][row] = false;
					  DFS(map, map[row][col - 1]);
				  }
				  break;
			  case 3:
				  if(col + 1 < horiz && map[row][col + 1].getIdx() == 0){
