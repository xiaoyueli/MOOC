...
class Ship:
    ...
    def update(self):
        ...
        if self.thrust:
            self.vel[0] += vector[0]/10
            self.vel[1] += vector[1]/10
        self.vel[0] *= 0.99
        self.vel[1] *= 0.99
    ...
    def shoot(self, sound):
        ...
        vel = [self.vel[0] + angle_to_vector(self.angle)[0]*10, self.vel[1] + angle_to_vector(self.angle)[1]*10]
...
def rock_spawner():
    global a_rock
    vel = [random.random()*5*random.choice([1, -1]), random.random()*5*random.choice([1, -1])]
    pos = [WIDTH/(random.random()*5), HEIGHT/(random.random()*5)]
    ang = random.random()*random.choice([1, -1])/5
    a_rock = Sprite(pos, vel, ang, ang, asteroid_image, asteroid_info)
